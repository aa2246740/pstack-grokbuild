...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using toml key `perf-issue` (`skills/setup-pstack/references/resolve-model.md`; omit `model` if missing/`inherit-parent`/`auto`); review the diff. Capture a post-fix trace.
