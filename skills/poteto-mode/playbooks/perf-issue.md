...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using toml key `perf-issue` (`skills/setup-pstack/references/resolve-model.md`; omit `model` if missing/`inherit-parent`/`auto`; `subagent_type: "perf-issue"` per `skills/setup-pstack/references/resolve-effort.md`; do not send `reasoning_effort` on `task`); review the diff. Capture a post-fix trace.
