...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using toml key `bug-fix` (`skills/setup-pstack/references/resolve-model.md`; omit `model` if missing/`inherit-parent`/`auto`) with a specific scope; review the diff.
