...
**Types.** Code-writing delegates and ad-hoc helpers: `subagent_type: "poteto-agent"` (or `pstack:poteto-agent` if the bare name collides). Routed workflow skills set their own type. `how` explorers/explainers/critics use `explore`. MCP-backed `why` investigators use `general-purpose` and forbid writes in the prompt. Independent verify uses `independent-verifier` plus a different `model` from the writer. `/no-comments` uses `comment-sicko`.

**Defaults for every `task` call.** `run_in_background: true` (that is also grok-build's schema default). File pointers, not inlined dumps. `model` from `~/.grok/pstack-models.toml` when set. Defaults: fast mechanical code `grok-4.6-fast-xhigh`, judgment and prose `claude-fable-5-thinking-max`, precisely specified instruction-following `gpt-5.6-sol-max`. `inherit-parent` or `auto` means omit `model`. Isolation is `none` unless the child must not touch the writer's tree, then `worktree`. Join with `get_task_output` (`task_ids`, positive `timeout_ms`). Reasoning effort is not a `task` field. Put it on the agent frontmatter (`effort`) when a role needs it. Independent verify is the `independent-verifier` agent (`effort: high`) plus a different `model`.
