...
**Depth.** Grok Build sets `MAX_SUBAGENT_DEPTH` to 1. Only this parent session may call `task`. A child that calls `task` fails. Fan-out (`how`, `swarm`, `arena`, `interrogate`, independent verify) stays in the parent. A code-writing `poteto-agent` child owns the diff directly. It does not spawn nested workers.

**Types.** Code-writing delegates and ad-hoc helpers: `subagent_type: "poteto-agent"` (or `pstack:poteto-agent` if the bare name collides). Routed workflow skills set their own type. `how` explorers/explainers/critics use `explore`. MCP-backed `why` investigators use `general-purpose` and forbid writes in the prompt. Independent verify uses `independent-verifier`. Send `model` only when toml `independent-verifier` is a detected slug different from the writer. Otherwise omit `model`. `/no-comments` uses `comment-sicko`.

**Defaults for every `task` call.** `run_in_background: true` (that is also grok-build's schema default). File pointers, not inlined dumps. Resolve `model` from `~/.grok/pstack-models.toml` per [`../setup-pstack/references/resolve-model.md`](../setup-pstack/references/resolve-model.md). Absent file, missing key, `inherit-parent`, or `auto`: omit `model`. Send a slug only when the toml names one confirmed this session. Isolation is `none` unless the child must not touch the writer's tree, then `worktree`. Join with `get_task_output` (`task_ids`, positive `timeout_ms`). Reasoning effort is not a `task` field. Put it on the agent frontmatter (`effort`) when a role needs it. Independent verify is the `independent-verifier` agent (`effort: high`) plus toml `independent-verifier` when that key is a detected slug different from the writer; otherwise omit `model`.
